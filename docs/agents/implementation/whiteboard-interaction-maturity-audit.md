...
- 一等对象：普通白板对象、文本/便签/LaTeX、图片、基础图形、坐标系、数组/栈/队列/双端队列、图、树和二叉树。
...
- 放置类工具行为稳定：文字/便签创建后进入编辑并在提交后回选择；结构和图片插入后选中新对象并回选择；图形工具可连续绘制，`Escape` 回选择。
...
结果：6 个测试文件通过，35 个测试通过；全量 `npm test` 868 个测试通过。
...
1. 先做 P0 压感笔触全链路，因为它涉及保存、复制、切分和导出，最容易产生数据不一致。
2. 再做 P0 结构内外交互和文本编辑尺寸一致性，因为它们是当前项目历史问题最多的交互区域。
3. 接着做 P0 属性栏回灌，确保 React 外壳显示可信。
4. 最后按 P1 处理触控板、右键图层、吸附和视觉反馈。

每个 P0 项都应先补测试，再改实现；若现有代码已满足，应只补回归测试和文档证据。
