...
元素点选由 `hit-query.getSelectableElementIdAtWorldPoint` + `pickElementIdAtPoint` 共同决定：

- 真实几何命中优先从**内容层**像素命中解析（跳过 `selection-overlay-layer` / `canvas-interaction-layer`），避免 Transformer 背板或交互护罩把内部矩形、椭圆、文字等吞掉。
- Konva 真实图形命中与边框附近是强命中；已选中元素的包围盒内部空白只是弱命中（方便拖拽）。
- 强命中优先于弱命中，再按 `zIndex` 取最上层。因此点选外框内部的任意元素时，不必先取消外框选中。

坐标系只把两条坐标轴和原点小范围加入 Konva 命中画布，左右键都可由命中节点回溯到坐标系元素；网格、刻度、标签、函数曲线和内部空白保持不可命中，避免挡住框选起手或坐标系上的其他元素。

- `drag-controller.js`：选区拖拽和 Konva node drag；拖动过程中同步结构浮动控件位置（线性单元格、树节点、图节点）。
...
网页工具先在 Konva 中绘制临时矩形，松开后校验尺寸并询问 `http(s)` 地址，提交为 `webpage` 元素，放置成功后默认切换到选择工具（工具锁定时遵循锁定状态）。`src/services/webpage-overlay-controller.js` 为每个网页元素创建 iframe DOM overlay，使用 eager 加载，随视口平移/缩放和元素旋转同步；网页和画布元素共用元素自身的 `zIndex`，`src/app/rendering/layer-order.js` 与 `layered-content.js` 把 Konva 内容拆成网页前后的 Canvas 层，网页 wrapper 使用对应的 DOM 层级，从而支持画布与多个网页任意交错。选择工具下，网页内容区是否接收点击按当前位置的真实元素覆盖关系决定：只有更高层画布元素的 hit graph 在该点命中时才拦截 iframe，空的 Canvas 层、外接矩形内部未命中区域和网页下方元素都不拦截；网页标题栏位于独立控制层，未选中的可选网页点击标题栏会完成选中，已选中且未锁定时拖动标题栏才移动，编辑/打开按钮和八向缩放柄只对已选中的网页开放。非选择工具下 iframe 和网页控制栏都让出指针事件，画布始终接收指针。图层栏选中画布元素后，`canvas-interaction-shield.js` 在其选区并集上创建透明 Konva 命中层，即使画布视觉上位于 iframe 下方，仍可从选区边界开始拖拽；结构元素（数组/矩阵/栈/队列/图/树）不进入护罩，避免挡住内部单元格/节点交互；拖拽进行中护罩扩展到视口，保证指针不会重新落入 iframe。多选拖拽只移动未锁定元素，点击、取消和 `pointercancel` 不产生历史。网页单选时禁用 Konva Transformer，移动、缩放在 overlay controller 中提交模型并各记录一条历史，同时实时同步 Konva 网页占位节点，避免拖拽期间旧位置残留一个网页框。页面拒绝嵌入时不在画布底部插入错误通知，用户可通过顶部的新窗口按钮打开；目标站点的 `X-Frame-Options` / CSP `frame-ancestors` 是浏览器安全限制，前端不能绕过。
